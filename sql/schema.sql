...
create table flights (
  ...
  target_galaxy int not null check (target_galaxy between 1 and 5),
  target_system int not null check (target_system between 1 and 500),
  target_position int not null check (target_position between 1 and 16),
  target_kind int,
...
create table automatic_resource_transfers (
  id bigserial primary key,
  user_id bigint references users not null,
  ...
  run_hour int not null check (run_hour between 0 and 23),
  run_minute int not null check (run_minute between 0 and 59),
...
create index automatic_resource_transfers_source_body_id_idx on automatic_resource_transfers (source_body_id);
create index automatic_resource_transfers_due_idx on automatic_resource_transfers (enabled, next_run_at, id);
